...
def small_zone_emphasis(matrix):
    ...
    for i in range(shape[1]):
        ...
        column = column / ((i + 1) * (i + 1))
        sum_col += column

    return sum_col / size


def large_zone_emphasis(matrix):
    # sum of values with discrete gray level
    size = np.size(matrix)
    shape = np.shape(matrix)
    sum_col = 0
...
def low_gray_level_zone_emphasis(matrix):
    import numpy as np
...
def high_gray_level_zone_emphasis(matrix):
    import numpy as np
...
def small_zone_low_gray_level_emphasis(matrix):
    size = np.size(matrix)
    shape = np.shape(matrix)
    all_values = 0
...
def small_zone_high_gray_level_emphasis(matrix):
    size = np.size(matrix)
    shape = np.shape(matrix)
    all_values = 0
...
def large_zone_low_gray_level_emphasis(matrix):
    size = np.size(matrix)
    shape = np.shape(matrix)
    all_values = 0
...
def large_zone_high_gray_level_emphasis(matrix):
    size = np.size(matrix)
    shape = np.shape(matrix)
    all_values = 0
    ...
    for i in range(shape[0]):
        for j in range(shape[1]):
            all_values += (
                    matrix[i, j] * pow(j + 1, 2) * pow(i + 1, 2))
...
def zone_size_non_uniformity(matrix):
    # sum of values with discrete gray level
    size = np.size(matrix)
    shape = np.shape(matrix)
    sum_col = 0
...
def normalised_zone_size_non_uniformity(matrix):
    # sum of values with discrete gray level
    size = np.size(matrix)
    shape = np.shape(matrix)
    size = pow(size, 2)
...
def zone_percentage(matrix1, matrix2):
    from GLSZM import get_voxels_number
